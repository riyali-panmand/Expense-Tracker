...
def setdate(Uid,win):
	def value():
		...
		if mon.get()=="January" and int(dt.get())==31:
			if int(y.get())%4==0:
				end_d = 29
			else:
				end_d = 28	
		...
		curent_date = now.strftime("%Y")+"-"+now.strftime("%B")+"-"+now.strftime("%d")
		cd = datetime.strptime(curent_date, "%Y-%B-%d")
		cd = datetime.date(cd)
		s=sd
		s=datetime.date(s)
		...
		for x in row:
			if x[4] == Uid:
				print(s,"\n","\n",x[1],"\n",x[2])
				if s>=x[1] and s<=x[2]:
					messagebox.showinfo("Error", "Date is already set")
					flag = 1
					break
		if flag==0:	
			cursor=conn.cursor()
